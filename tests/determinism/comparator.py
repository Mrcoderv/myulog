...
class ByteComparator:
    ...
    def compare(self, events1: List[Dict[str, Any]], events2: List[Dict[str, Any]]) -> List[Difference]:
    ...
    def _compare_events(
        self, event_index: int, event1: Dict[str, Any], event2: Dict[str, Any], path: str = ""
    ) -> List[Difference]:
        ...
        for key in keys1 - keys2:
            ...
            differences.append(
                Difference(
                    event_index=event_index, field_path=field_path, value1=event1[key], value2=None, diff_type="missing"
                )
            )
        ...
        for key in keys2 - keys1:
            ...
            differences.append(
                Difference(
                    event_index=event_index, field_path=field_path, value1=None, value2=event2[key], diff_type="extra"
                )
            )
    ...
    def _compare_values(self, event_index: int, field_path: str, val1: Any, val2: Any) -> List[Difference]:
        ...
        if type(val1) is not type(val2):
            differences.append(
                Difference(event_index=event_index, field_path=field_path, value1=val1, value2=val2, diff_type="type")
            )
            return differences
        ...
        if isinstance(val1, dict):
            ...
            if repr(val1) != repr(val2):
                differences.append(
                    Difference(
                        event_index=event_index, field_path=field_path, value1=val1, value2=val2, diff_type="value"
                    )
                )
        else:
    ...
    def _compare_lists(self, event_index: int, field_path: str, list1: List[Any], list2: List[Any]) -> List[Difference]:
        ...
        for i in range(min_length):
            ...
            element_diffs = self._compare_values(event_index, element_path, list1[i], list2[i])
