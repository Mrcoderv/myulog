...
class GenerateCVLog(GenerateLog):
    def __init__(self, fields: list[str], size: int, seed: int,
                 input_params: list[str] | None,valid_params: list[str]) -> None:
        ...
        self.accelerators = ["NVIDIA A100", "NVIDIA V100", 
                             "Google TPU v3", "Intel Xeon", "AMD MI100"]
        self.levels = ["device","host","cluster","edge","cloud"]

        self.metrics = ["accuracy", "loss", "precision", "recall", 
                        "f1_score", "mAP","fps", "iou", "ap50", "ap"]
        
        self.phases = ["ingest", "preprocess", "inference", 
                       "postprocess", "eval", "serve", "track", "pose"]
    ...
    def generate_metrics(self):
        ...
        metrics["iou"] = round(min((metrics["f1_score"] + metrics["recall"]) / 2 +
                                    self.generate_float(-0.05, 0.05), 1), 4)

        metrics["ap50"] = round(min(metrics["iou"] +
                                        self.generate_float(-0.05, 0.05), 1), 4)
        
        metrics["ap"] = round(min((metrics["precision"] + metrics["recall"]) / 2 +
                                    self.generate_float(-0.05, 0.05), 1), 4)

        metrics["map"] = round(min((metrics["ap"] + metrics["ap50"]) / 2 +
                                    self.generate_float(-0.02, 0.02), 1), 4)
